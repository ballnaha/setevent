...
// Design interface aligned with Schema
interface Design {
    id: string;
    title: string;
    category: string;
...
export default function DesignsAdminPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const url = editId ? `/api/admin/designs/${editId}` : "/api/admin/designs";
    ...
    const handleDeleteConfirm = async () => {
        ...
        setDeleting(true);
        try {
            await fetch(`/api/admin/designs/${deleteId}`, { method: "DELETE" });
            fetchDesigns();
        } catch (error) {
    ...
    // Filter designs by category tab
    ...
    const getCategoryCount = (cat: string) => {
        if (cat === "All") return designs.length;
                                ...
                                <TableRow>
                                    <TableCell colSpan={7} align="center" sx={{ py: 8, color: 'text.secondary' }}>
                                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                                            <Brush2 size="48" color="rgba(0,0,0,0.2)" variant="Bulk" />
                                ...
                                filteredDesigns.map((design) => (
                                    <TableRow key={design.id} hover>
                                        ...
                                        <TableCell>
                                            <Chip
                                                label={design.status}
                                                color={design.status === 'active' ? 'success' : 'default'}
                                                size="small"
                                            />
                                        </TableCell>
                                        <TableCell align="right">
                                            <IconButton onClick={() => handleOpen(design)}>
                                                <Edit size="18" color="#3b82f6" />
                                            </IconButton>
                                            <IconButton onClick={() => handleDeleteClick(design.id)}>
            ...
            >
                ...
                <DialogContent>
                    <Typography sx={{ fontFamily: 'var(--font-prompt)' }}>
                        คุณต้องการลบ design นี้หรือไม่? การดำเนินการนี้ไม่สามารถย้อนกลับได้
                    </Typography>
                </DialogContent>
                <DialogActions>
                    <Button
